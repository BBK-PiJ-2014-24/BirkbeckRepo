...
public class TaskManager implements Runnable {
	...
	private Queue<Runnable> taskQueue;    // This is a Queue of RUNNABLES WHICH HAVE NOT BEEN THREADED.
    ...
    public TaskManager() {
	  ...
	  this.running = true;     // flag to ShutDown
    }
    ...
    public synchronized void add(Runnable task) {
	  if (running) {
		...
		this.taskQueue.offer(task);    // ADD RUNNABLE TO END OF QUEUE (NOT YET PLACED IN THREAD)
		System.out.println("Waking up " + task + " task manager's thread.");
		notifyAll();    ///  NOTIFY MSG...
	  } else {
    ...
    public void run() {
	  while (running) {
		while (!taskQueue.isEmpty()) {
		    Runnable next = taskQueue.poll();   // Take NEXT RUNNABLE FROM QUEUE
